...
Device.prototype.connectDeviceAsync = function(deviceInterface, timeout) {
    const self = this;
    ...
    return new Promise(async function(resolve, reject) {
        if(deviceInterface) {
            ...
            deviceInterface.Connect(function(err) {
                ...
                if(!err) {
                    self.waitForServicesResolved(timeout).then(() => resolve()).catch(err => reject(err));
                } else {
...
To do Characteristics access properly, this class should traverse down all /serviceXXXX/charXXXX paths, fetch their UUIDS
and build a map of those by uuid, because the handles might change depending on firmware versions or other things
 */