...
MiKettleMqttHandler.prototype.updateStateTopic = function(state) {
    ...
    if(stringifiedState !== this.lastState) {
        ...
        this.mqttClient.publish("kettle/" + this.device.id + "/state", state.current_temperature.toString(), err => {
            if(err) {
                console.error(err);
            }
        });
        this.mqttClient.publish("kettle/" + this.device.id + "/attributes", JSON.stringify({
            action: state.action,
            mode: state.mode,
