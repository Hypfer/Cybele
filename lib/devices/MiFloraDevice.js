...
MiFloraDevice.prototype.poll = async function() {
    ...
    try {
        if(!this.interfaces.device) {
            this.interfaces.device = await this.getDBusInterfaceAsync(
    ...
    try {
        await this.connectDeviceAsync(this.interfaces.device, 5000, 5); //TODO: configurable?
    } catch(e) {
        console.error(e);
        return this.queuePolling();
    }
    ...
    try {
        if(!this.interfaces.firmware) {
            this.interfaces.firmware = await this.getDBusInterfaceAsync(
                this.pathRoot + "/dev_" + this.macInDbusNotation + "/service0031/char0037",
                "org.bluez.GattCharacteristic1"
            );
        }

        if(!this.interfaces.deviceMode) {
            this.interfaces.deviceMode = await this.getDBusInterfaceAsync(
                this.pathRoot + "/dev_" + this.macInDbusNotation + "/service0031/char0032",
                "org.bluez.GattCharacteristic1"
            );
        }

        if(!this.interfaces.sensorData) {
            this.interfaces.sensorData = await this.getDBusInterfaceAsync(
                this.pathRoot + "/dev_" + this.macInDbusNotation + "/service0031/char0034",
                "org.bluez.GattCharacteristic1"
            );
        }
    } catch (e) {
    ...
    try {
        await this.writeCharacteristicAsync(this.interfaces.deviceMode, MiFloraDevice.LIVE_MODE_CMD, "request")
    } catch(e) {
        console.error(e);
        return this.queuePolling();
    }
    ...
    try {
        fwInfo = MiFloraDevice.PARSE_FIRMWARE_CHARACTERISTIC(await this.readCharacteristicAsync(this.interfaces.firmware));
    ...
    try {
        reading = MiFloraDevice.PARSE_DATA(await this.readCharacteristicAsync(this.interfaces.sensorData));
    } catch(e) {
        console.error(e);
        return this.queuePolling();
    }
