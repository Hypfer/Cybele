...
class BodyMetrics {
     ...
     * @param {number} options.height //in cm
     * @param {number} options.age
     * @param {"M"|"F"} options.sex
     */
    constructor(options) {
    ...
    getFatPercentage(weight, impedance) {
    ...
    getWaterPercentage(weight, impedance) {
    ...
    getIdealWeightScale() {
        return this.getBMIScale().map(v => {
            return (v * this.height) * this.height / 10000;
        });
    }

    //Get fat mass to ideal (guessing mi fit formula)
    getFatMassToIdeal(weight, impedance) {
    ...
    getProteinPercentage(weight, impedance) {
    ...
    getAllMetrics(weight, impedance) {
        const measurements = {
            ...
            IdealWeight: {
                ...
                scale: this.getIdealWeightScale().map(v => {
                    return v.toFixed(2);
                })
            },
