...
MiBodyScaleDevice.prototype.initialize = function(callback) {
    this.mqttClient.publish("homeassistant/sensor/body_scale_" + this.id + "/config", JSON.stringify({
        "state_topic": "body_scale/" + this.id + "/state",
        "json_attributes_topic": "body_scale/" + this.id + "/attributes",
...
MiBodyScaleDevice.prototype.handleAdvertisingForDevice = function(prop) {
    ...
    if(
        prop.ServiceData &&
        prop.ServiceData.UUID === "0000181b-0000-1000-8000-00805f9b34fb" &&
        Buffer.isBuffer(prop.ServiceData.data)
    ) {
        const parsedData = this.parseServiceData(prop.ServiceData.data);

        if(parsedData) {
            this.mqttClient.publish("body_scale/" + this.id + "/state", parsedData.weight.toFixed(2));
            this.mqttClient.publish("body_scale/" + this.id + "/attributes", JSON.stringify(parsedData.attributes));
        }
    }
};
